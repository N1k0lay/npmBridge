...
### Постфикс (2026-05-25): устаревший diff всё равно можно отметить как перенесённый

**Проблема**: если diff уже был физически отправлен в сеть, но до подтверждения в UI в `storage/` появлялись новые изменения, API блокировал `confirm_transfer` ошибкой «Diff устарел». Это ломало реальный workflow: пользователь не мог зафиксировать уже выполненный перенос.

**Исправление**:
1. `webapp/src/app/api/diff/route.ts`: подтверждение переноса не блокируется только из-за устаревания diff, но запрещается для diff, которые уже перекрыты более новым `transferred` checkpoint
2. `webapp/src/lib/store.ts`: новый diff получает `sinceTime` от последнего `transferred` diff, а не от последнего любого diff
3. `scripts/create_diff.py`: логика `sinceTime` и журналирования тоже опирается на последний `transferred` checkpoint
4. `webapp/src/components/DiffPanel.tsx`: кнопки переноса скрываются у diff, для которых уже существует более новый `transferred` diff

### Постфикс (2026-05-18): incremental diff пропускал реальные новые пакеты

**Проблема**: создание diff могло завершаться сообщением «Новых пакетов не найдено», хотя в `storage/` уже были файлы, отсутствующие в `frozen/`.
**Причина**: `scripts/create_diff.py` определял изменения по `mtime > createdAt последнего diff`, а не по реальной разнице `storage/` и `frozen/`. Если файлы попадали в `storage/` со старым mtime, они ошибочно исключались.

**Исправление**:
1. `scripts/create_diff.py`: логика построения diff переведена на сравнение `storage/` против `frozen/`
...
**Причина**: `webapp/src/app/api/diff/[id]/download/route.ts` читал весь `.tar.gz` через `fs.readFile()`, а Node.js выбрасывает `ERR_FS_FILE_TOO_LARGE` для файлов больше 2 GiB.

**Исправление**:
1. `webapp/src/app/api/diff/[id]/download/route.ts`: отдача переведена на `createReadStream()` + `Readable.toWeb()`
2. В ответ добавлен `Accept-Ranges: bytes`, а `Content-Length` берётся из `stat()` файла
3. Проверено на проде URL `diff_2026-04-30T07-02-24-616Z/download` для архива 58.15 GB — endpoint отвечает `200 OK`
