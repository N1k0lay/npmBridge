...
- [x] Long-running создание diff переведено на фоновую задачу с polling-прогрессом и приблизительной ETA для больших архивов
- [x] Для создания diff добавлены live-логи и ручная остановка задачи из UI
- [x] Отмена создания diff безопасна: частично собранный архив не остаётся в diff_archives/
...
- [x] `/api/diff/download/[id]` потоково отдаёт большие архивы и не падает на файлах > 2 GiB
...
- [x] Устаревший diff можно отметить как перенесённый постфактум без отката baseline snapshot назад
- [x] Подтверждать перенос можно только до следующего `transferred` checkpoint; старые diff после checkpoint становятся неактуальны для отметки
