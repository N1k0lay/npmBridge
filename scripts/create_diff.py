#!/usr/bin/env python3
"""
Скрипт создания diff на основе времени последнего diff.
Находит все .tgz файлы в storage/, новее sinceTime (из последнего diff JSON).
Архив создаётся без frozen директории.
"""
...
def get_last_diff_time() -> str | None:
    """
    Читает JSON файлы из diff_archives/, находит последний завершённый diff
    и возвращает его createdAt как sinceTime для нового diff.
    """
    archives_path = Path(DIFF_ARCHIVES_DIR)
    if not archives_path.exists():
        return None

    # Берём все JSON файлы (не .tmp) в diff_archives/
    json_files = sorted(archives_path.glob('diff_*.json'))
    if not json_files:
        return None

    # Ищем последний diff с filesCount > 0 (не пустой)
    for json_file in reversed(json_files):
        try:
            with open(json_file) as f:
                data = json.load(f)
            # Берём любой diff (даже устаревший) — нам важно createdAt
...
def get_diff_files(since_time: str | None) -> list[tuple[str, Path]]:
    """
    Получение списка .tgz файлов в storage/, созданных после since_time.
    Если since_time=None — возвращает все .tgz файлы.
    """
    ...
    # Инкрементальный diff — используем find -newermt
    # since_time может быть в формате ISO 8601
    # Преобразуем в формат для find (YYYY-MM-DD HH:MM:SS)
    try:
        dt = datetime.fromisoformat(since_time.replace('Z', '+00:00'))
        # find понимает формат YYYY-MM-DD HH:MM:SS в UTC если без TZ
    ...
    try:
        ...
        log('ERROR', f'find command failed: {e}, falling back to full scan')
        # Fallback: полный скан с фильтрацией по mtime
...
def main():
    ...
    # Определяем sinceTime из последнего diff
    ...
    # Возвращаем результат (без sidecar JSON — store.ts создаст его сам)
    result = {
        "diffId": DIFF_ID,
