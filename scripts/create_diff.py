...
def has_transfers(diff_meta: dict[str, object]) -> bool:
    transfers = diff_meta.get('transfers')
    return isinstance(transfers, dict) and len(transfers) > 0


def is_checkpoint_diff(diff_meta: dict[str, object]) -> bool:
    return diff_meta.get('status') == 'transferred' or has_transfers(diff_meta)


def get_last_checkpoint_diff_time() -> str | None:
    ...
    for json_file in reversed(json_files):
        try:
            ...
            if not is_checkpoint_diff(data):
                continue
...
def ensure_baseline_manifest(created_at: str, since_time: str | None) -> dict[str, object] | None:
    ...
    for diff_meta_path in reversed(diff_meta_files):
        ...
        if not diff_meta.get('id'):
            continue
        if not is_checkpoint_diff(diff_meta):
            continue
...
def get_diff_files(created_at: str) -> tuple[list[tuple[str, Path]], dict[str, object]]:
    ...
    since_time = get_last_checkpoint_diff_time()
...
def main():
    ...
    if since_time:
        ...
        log('INFO', 'Full diff (no checkpoint diffs found)')
